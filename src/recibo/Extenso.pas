...
Function TACBrExtenso.Poem_Zeros(const Texto: AnsiString; const Tamanho: Integer
  ): AnsiString;
begin
  Result := PadR(Trim(Texto),Tamanho,'0') ;
end;

Function TACBrExtenso.IntToStrZero(const NumInteiro: Int64; Tamanho: Integer
  ): AnsiString;
begin
  Result := '' ;
  try
...
function TACBrExtenso.padR(const AString: AnsiString; const nLen: Integer;
  const Caracter: AnsiChar): AnsiString;
begin
